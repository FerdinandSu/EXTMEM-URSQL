...
#define _EXTMEM_SPRINT_BUFFER_SIZE 40
...
int dropBlockOnDisk(unsigned long long addr)
{
	char filename[40];

	sprintf_s(filename, _EXTMEM_SPRINT_BUFFER_SIZE, "data/%lld.blk", addr);
...
unsigned char* readBlockFromDisk(unsigned long long addr, Buffer* buf)
{
	char filename[40];
...
int writeBlockToDisk(unsigned char* blkPtr, unsigned long long addr, Buffer* buf)
{
	char filename[40];

	sprintf_s(filename, _EXTMEM_SPRINT_BUFFER_SIZE, "data/%llu.blk", addr);
	FILE* fp;

	if (fopen_s(&fp, filename, "w") && fp!=0)
	{
		perror("Writing Block Failed!\n");
		return -1;
	}

	for (unsigned char* bytePtr = blkPtr; bytePtr < blkPtr + buf->blkSize; bytePtr++)
